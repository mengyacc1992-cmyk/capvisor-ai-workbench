...
const App: React.FC = () => {
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      const errorMsg = error?.message || '登录失败，请检查：\n1. 网络连接是否正常\n2. 后端服务是否运行在 http://localhost:8787\n3. 账号密码是否正确（demo@capvisor.ai / capvisor123)';
      alert(errorMsg);
    } finally {
      setAuthing(false);
    }
  ...
  const handleStartGeneration = async () => {
    ...
    try {
      ...
      const errorMsg = error?.message || '生成选题失败';
      // 提供更详细的错误信息
      alert(`生成选题失败：${errorMsg}\n\n如果网络无法访问 Google API，系统会自动使用降级方案。\n请检查：\n1. 网络连接\n2. 后端服务是否正常运行\n3. 浏览器控制台是否有更多错误信息`);
    } finally {
      setLoading(false);
    }
  };
